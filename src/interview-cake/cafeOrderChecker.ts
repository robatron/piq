...
export default (
    ...
    for (let si = 0; si < servedOrders.length; si++) {
        const takeOutOrder = takeOutOrders[ti];
        const dineInOrder = dineInOrders[di];
        const servedOrder = servedOrders[si];
