/**
 * @lc app=leetcode id=975 lang=typescript
 *
 * [975] Odd Even Jump
 *
 ...
 *   smallest such index `j`. It may be the case that for some index i, there
 *   are no legal jumps.
 *
 ...
 *
 * _Return the number of *good* starting indices._
 *
 * Example 1:
 *
 ...
 *     `arr[2]` is the smallest among `arr[1]`, `arr[2]`, `arr[3]`, `arr[4]`
 ...
 *       `arr[1]` is the smallest value in `[arr[1], arr[2], arr[3], arr[4]]`
 *       that is greater than or equal to `arr[0]`.
 *     - During our 2nd jump (even-numbered), we jump from `i = 1` to `i = 2`
 *       because `arr[2]` is the largest value in `[arr[2], arr[3], arr[4]]`
 *       that is less than or equal to `arr[1]`. `arr[3]` is also the largest
 *       value, but 2 is a smaller index, so we can only jump to `i = 2` and not
 *       `i = 3`
 *     - During our 3rd jump (odd-numbered), we jump from `i = 2` to `i = 3`
 *       because `arr[3]` is the smallest value in `[arr[3], arr[4]]` that is
 *       greater than or equal to `arr[2]`.
...
const oddEvenJumps = (arr: number[]): number => {
    ...
    for (let startIdx = 0; startIdx < len; startIdx++) {
        let jumpNum = 1;
        let jumpIdx: number = startIdx;

        // Keep jumping until we reach the end or there's no legal jumps left
        while (jumpIdx !== null) {
            if (jumpIdx === len - 1) {
                successCt++;
                break;
            ...
            const isOddJump = Boolean(jumpNum % 2);
            jumpIdx = getNextJumpIdx(arr, jumpIdx, isOddJump);
            jumpNum++;
        }
    }
...
// const oddEvenJumpsDP = (arr: number[]): number => {};

/**
 * Return the index of the next valid jump or null if none can be found. The
 * next valid jump is the smallest item >= to odd start jumps, and the largest
 * item >= to even start jumps
 */
const getNextJumpIdx = (
    arr: number[],
    startIdx: number,
    isOdd: boolean,
): number => {
    const startVal: number = arr[startIdx];
    let nextJumpIdx: number = startIdx === arr.length - 1 ? startIdx : null;
    let nextJumpVal: number = isOdd ? Infinity : -Infinity;

    for (let i = startIdx + 1; i < arr.length; i++) {
        const curVal: number = arr[i];
        const isCandidate: boolean = isOdd
            ? curVal >= startVal && curVal < nextJumpVal
            : curVal <= startVal && curVal > nextJumpVal;
